...
async fn chat_completions_inner(
    ...
    // tokenize. The exact count makes the `prefill_tokens` active-load gauge
...
/// a registered request always shows up as "load > 0" in the
/// `sgl_router_active_load{kind="prefill_tokens"}` gauge.
///
/// Only reached when the ingress did not tokenize — the caller prefers the
/// exact `request_tokens` count. This is a coarse approximation: body length
/// in bytes divided by [`CHARS_PER_TOKEN_ESTIMATE`].
