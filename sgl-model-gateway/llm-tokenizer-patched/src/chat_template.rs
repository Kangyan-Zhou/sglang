...
impl ChatTemplateProcessor {
    ...
    pub fn apply_chat_template(
        ...
        // Match HuggingFace Python's Jinja2 environment settings.
        // Without these, templates that rely on implicit whitespace stripping
        // (e.g., GLM-5 using `{% set` instead of `{%- set`) produce variable
        // whitespace that breaks radix cache prefix matching.
        ...
        // Use Value::UNDEFINED for missing optional params so they are truly
        // "undefined" in the template context, matching HuggingFace Python behavior.
        // Many chat templates use `{% if tools is defined %}` guards — passing
        // null (none) instead of undefined would bypass those guards since `none`
        // IS defined, causing `tools | length` to fail.
